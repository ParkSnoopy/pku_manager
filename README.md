...
- **Calendar** shows five weeks at once. Scroll vertically through week rows; its heading follows the month owning most visible date cells. Month boundaries use divider lines instead of different cell backgrounds, and today is labeled **TODAY**. New schedules begin with a blank title, use the current theme color by default, and are applied with **Save** or discarded with **Cancel**. All-day schedules use 23:59 Beijing time as their deadline. Schedules can include a note, fixed custom color, time, and related class; **Use theme color** immediately removes a selected custom color. Select a related schedule to reveal its class in Timetable; press and hold it to edit. Its landscape right pane lists every class scheduled tomorrow, each class's upcoming schedules, and then unassociated schedules after a separator. Schedule entries show **DDL**, and navigation targets briefly flash without remaining selected.
