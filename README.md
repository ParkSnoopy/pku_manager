...
- Vertically touching cells for the same course on one weekday appear as one block. Select the block to edit all of its source records together, or select an empty cell to add a course. Imported workbook bytes remain unchanged; edits are stored separately.
- The course editor can choose any palette color and add a strong importance outline. Manual colors carry a lock/palette marker; locked colors survive **Roll colors**, while an unlocked manual color returns to generated colors on the next roll.
- On pointer devices, hover for one second to show details beside the pointer.
- In landscape, the right pane lists every course's next start and the time remaining. Selecting a timetable block replaces that list with an editable details pane instead of opening a dialog.
