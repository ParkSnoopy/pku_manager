...
- **Timetable** uses a transparent grid canvas while retaining a configurable header/index color. Dark mode blends the header, index, and class-cell colors into the dark surface. Each upcoming personal schedule stays in its own box, with its related class appended below in a separately styled cell and spacing between schedule groups. Every right-pane schedule and class shows its remaining time as **DDL**. Hover a right-pane schedule to see its details, select it to reveal it in Calendar, or right-click/press and hold to edit it.
- **Calendar** gives adjacent-month cells a light-grey background and marks today with an icon beside its date. New schedules begin with a blank title, use the current theme color by default, and are applied with **Save** or discarded with **Cancel**. All-day schedules use 23:59 Beijing time as their deadline. Schedules can include a note, fixed custom color, time, and related class; **Use theme color** immediately removes a selected custom color. Select a related schedule to reveal its class in Timetable; press and hold it to edit. Its landscape right pane lists every class scheduled tomorrow, each class's upcoming schedules, and then unassociated schedules after a separator. Every entry shows **DDL**, and navigation targets briefly flash without remaining selected.
