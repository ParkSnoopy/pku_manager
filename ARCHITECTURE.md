...
- `features/calendar/calendar_page.dart` renders an independent navigable personal month containing only color-coded user-created schedules. Schedules default to all-day, update interactively, and may store one related class source ID. Landscape Timetable shows upcoming personal schedules with exact date/time labels and swaps that pane for the shared inline course editor only when a timetable block is selected directly. Landscape Calendar lists every class scheduled tomorrow; classes are never projected into the month. Side-pane entries reveal and highlight their target without opening an editor.
...
Course colors default to deterministic values selected from a persisted Roll palette and seed using the immutable workbook-parsed class name. The initial palette exactly matches upstream `palette.json` at revision `eaacca788e246a18c36ea013ced2bed6b62bd995`; its malformed `##ffafcc` palette is not normalized. Optional manual colors and configurable importance outlines are persisted by active-source identity; outlines default to red at 1.5 logical pixels. Course names and classroom text use the same fixed 22.5-pixel base size, the room starts with two spaces, and padded one-line labels truncate within each block. Remarks use a smaller third line with explicit spacing. A successfully parsed structured tutorial remark is consumed when its separate tutorial class is created, while ambiguous source text remains available for review. Locked colors survive rolls, while unlocked manual colors return to generated colors on the next roll. Vertically touching courses with the same immutable source name render as one block even after display-name edits. Foreground contrast is calculated from the chosen background. After 100 ms hover, a pointer-following overlay shows class start/end times, full course details, and related personal schedules.
...
| Appearance | Typed accent, language, font scale/weight, Roll palette/seed, and source-bound course appearance rows | Explicit Settings, editor, and Roll interactions | Apply immediately and transactionally; reject styles outside the active schedule. |
| User courses | Typed weekday rows linked to the active immutable source | Empty-cell additions | Insert, edit, or delete independently from source and parsed rows. |
| Calendar schedules | Typed title, date/time, all-day flag, and optional related-class source ID independent from timetable sources | User-created personal schedules | Apply interactive edits without importing timetable classes. |
