...
- `features/calendar/calendar_page.dart` renders an independent navigable personal month and an event-only landscape pane. Neither derives entries or class information from timetable groups. Landscape Timetable shows the upcoming schedule and swaps it for the shared inline editor on selection. Portrait timetable layouts use the editor inside a dialog.
