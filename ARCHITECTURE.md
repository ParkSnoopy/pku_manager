...
| Window adapter | `lib/ui/app_window_controller.dart` | Current | Configure the desktop launch window, expose synchronized full-screen state, and apply the persisted close-to-tray or close-app action without leaking plugin calls into features. |
...
- `timetable_export.dart` generates complete five-weekday PNG and XLSX files from the same grouped-span geometry, shared theme-color rules, user-selected typography, course content, and appearance map as the screen. Both formats intentionally render every class fully opaque regardless of current-week membership. PNG adds 12 logical units of canvas padding and renders the complete table at 4× resolution; XLSX retains four role rows per period underneath merged course blocks with matching dimensions, fonts, and alignment.
- `features/settings/appearance_controller.dart` owns persistent light/dark mode, accent and theme blending, language, desktop close action, font family, application and timetable font scales, font weight, timetable header/index color, automatic course-text contrast, and timetable-palette state. `settings_page.dart` applies those values immediately, groups typography controls, exposes the editable Custom palette and named upstream palettes as swatches instead of indexes, and advances font family and language through one cycling action each.
...
Widgets consume domain projections supplied by the controller. They do not filter frequency with local string checks. Every meeting block is shown; meetings outside current parity render at quarter opacity without cell text. The `Timetable` domain projection identifies classes whose weekday/period ranges overlap in any week their frequencies can coincide; screen and exports override manual importance outlines with a red conflict outline, while odd/even-only alternation remains valid. In dark mode, one shared presentation rule blends weekday, index, and class backgrounds toward the active dark surface and derives readable foregrounds. Confirmed class deletion atomically removes the edited imported or user-created meeting rows plus completions, issues, and appearance records while preserving immutable source bytes; related calendar schedules survive with their class association cleared.
