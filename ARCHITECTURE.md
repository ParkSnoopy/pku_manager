...
- `features/calendar/calendar_page.dart` renders an independent navigable personal month. It does not derive entries from timetable groups. Landscape Timetable shows the upcoming schedule and swaps it for the shared inline editor on selection. Portrait timetable layouts use the editor inside a dialog.
