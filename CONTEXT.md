...
- Calendar marks today with an icon beside the date number. Primary activation of a related schedule reveals its source class in Timetable; press-and-hold edits the schedule.
...
- Right-pane schedules and classes are separate styled cells rather than mixed text in one cell. Every schedule and class cell shows time remaining through the unified `DDL` expression.
