...
- Roll uses a user-selectable palette. The initial five colors are copied exactly from `palette.json` at `ParkSnoopy/pku-elective-prettify` revision `eaacca788e246a18c36ea013ced2bed6b62bd995`; the upstream palette containing the malformed literal `##ffafcc` is not normalized or offered.
...
- The application version is `0.0.6`. Throughout `0.0.x`, the development database keeps `PRAGMA user_version = 0`; no backward-compatibility or migration code is retained until an explicit schema-version bump is requested.
