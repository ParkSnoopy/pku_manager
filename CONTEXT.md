...
- Settings provide immediate 80%–150% application text scaling, 100%–200% timetable text scaling, Serif/Sans Serif selection, and Thin–Black weight controls. Timetable classroom text matches class-name size and starts with two spaces.
...
- Calendar presents an independent, color-coded personal month view for user-created schedules. All date cells share one background, with foreground divider lines marking intrinsic month boundaries independently of the superior visible month. Schedule boxes span the full cell width without left or right margins. All-day schedules normalize their deadline to 23:59 Beijing time on the selected day. Schedules derive their default color from the active theme; selecting a custom color fixes it until **Use theme color** clears it. Schedule note and optional selected color persist with its title, date/time, all-day flag, and optional class relation.
- Right-pane schedules and classes are separate styled cells rather than mixed text in one cell. Schedule cells show time remaining through the unified `DDL` expression; course cells never show `DDL`.
