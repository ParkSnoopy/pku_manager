...
- Landscape Timetable layouts show upcoming personal schedules with exact dates and times but without class information in the right pane; selecting a course changes that pane into the editor. Landscape Calendar layouts show every class scheduled tomorrow without personal schedule information. Portrait timetable selection uses a dialog.
...
- Selecting a side-pane schedule or class reveals and highlights its target without opening an editor.
...
- Course names and classroom text use the same fixed 22.5-pixel base size, and both remain padded and truncate within the course block. Classroom text starts with two spaces. Parsed remarks appear as a separate spaced line; a structured remark successfully projected as a separate class is removed from both source and projected display records.
- Calendar presents an independent, color-coded personal month view for user-created schedules. It never adds timetable classes to the month. Its landscape right pane lists all classes scheduled tomorrow.
- The application version is `0.0.8`. Throughout `0.0.x`, the development database keeps `PRAGMA user_version = 0`; no backward-compatibility or migration code is retained until an explicit schema-version bump is requested.
