...
- Landscape Timetable layouts show the upcoming schedule in the right pane; selecting a course changes that pane into the editor. Portrait timetable selection uses a dialog.
...
- Calendar presents an independent personal month view and never imports timetable classes automatically.
- The application version is `0.0.3`. Throughout `0.0.x`, the development database keeps `PRAGMA user_version = 0`; no backward-compatibility or migration code is retained until an explicit schema-version bump is requested.
