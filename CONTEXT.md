...
- The timetable grid has no explicit canvas fill. Its weekday row and period-index column share one persisted configurable color. Dark mode blends the weekday row, index column, and class-cell colors into the dark application surface and uses readable light text. A default-off setting globally enables automatic text-color inversion against arbitrary course and schedule backgrounds; while disabled, colored content uses the active theme's standard black or white text.
