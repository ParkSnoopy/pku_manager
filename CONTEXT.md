...
- Landscape Timetable layouts show the upcoming schedule in the right pane; selecting a course changes that pane into the editor. Landscape Calendar layouts show upcoming personal schedules without timetable class information. Portrait timetable selection uses a dialog.
...
- Calendar presents an independent personal month view and upcoming-schedules pane, neither of which imports or describes timetable classes.
- The application version is `0.0.4`. Throughout `0.0.x`, the development database keeps `PRAGMA user_version = 0`; no backward-compatibility or migration code is retained until an explicit schema-version bump is requested.
