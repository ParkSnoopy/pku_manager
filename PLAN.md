...
- The Flutter application is at version `0.0.12`; all `0.0.x` builds keep schema version 0 without compatibility code.
...
- `lib/features/calendar/calendar_page.dart`: independent color-coded month calendar for personal schedules.
...
- Mark every class involved in a same-week weekday/period overlap with a red outline on screen and in exports; do not treat odd/even alternation as a conflict.
...
- Match timetable shape, dimensions, alignment, meal breaks, aspect fitting, and 4× PNG export resolution to the `pages` reference. Keep start/end time out of index cells and in hover details. Package each selectable CJK family as one Static Super OTC instead of separate region/weight files.
...
- Apply valid course edits immediately without a Save action. Put confirmed permanent class deletion at the bottom of the course editor's scrollable form, separate from its fixed Close footer, and retain related schedules without their deleted class association. Use a larger Cancel/Save draft editor for personal schedules, with blank new titles, notes, colors, all-day/time controls, and optional related classes.
- Expose immediate application and timetable text scales, Serif/Sans Serif selection across course, weekday, and period-index cells, and every Thin–Black weight in one font section. Render classroom and class names at the same base size with two leading room spaces; wrap prefix-free remarks.
...
- Group vertically touching identical weekday meetings into one block and atomically edit all retained source identities without mutating imported source bytes; export the complete timetable as PNG or XLSX with displayed appearance and content but without non-current-week transparency.
...
- Let Settings choose pastel or arbitrary theme accents, optionally blend the accent into the theme, edit the Custom timetable palette, and hide the Roll colors rail action. Use fixed padded course typography.
- Add an independent user-controlled, color-coded Calendar with five vertically scrollable week rows, cell-superiority month headings, vertical and partial horizontal month-boundary lines, and a `TODAY` label. In landscape, Timetable appends related classes beneath upcoming schedules, separates schedule groups, places each `DDL` countdown in a right-aligned span with 1.25× value text, exposes pointer-following hover details, navigates on primary activation, and edits on secondary-click or press-and-hold; Calendar lists every class scheduled tomorrow, each class's schedules, and unassociated schedules last. Show DDL only on schedule entries. Primary activation of a Calendar schedule reveals its related timetable class, while press-and-hold edits. Side-pane navigation uses a transient flashing target outline without opening editors. Escape restores transient page presentation without reverting data or settings. Never project timetable classes into the month. Keep dialog editing for portrait timetable layouts.
- Add a **教学网** rail action that opens the exact PKU Teaching Network URL in the platform default browser.
- Launch desktop windows at 1600×900 while retaining resize support, and expose synchronized full-screen toggling through the rail and F11.
- Let desktop Settings choose whether the native close action hides to a restorable system-tray icon or closes the app; retain close-app behavior as the default and keep the window visible if tray setup fails.
